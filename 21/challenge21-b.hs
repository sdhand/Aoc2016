...
rotateT x pass = head $ filter (\y -> rotateB x y == pass) allComb
    where allComb = take (length pass) $ iterate (rotateR 1) pass
...
move y x pass = let (a,b) = splitAt y (delete (pass !! x) pass) in a++[pass !! x]++b
...
decode pass command
    ...
    | (command !! 1) == "left" = rotateR (read $ command !! 2) pass
    | (command !! 1) == "right" = rotateL (read $ command !! 2) pass
    | (command !! 1) == "based" = rotateT (head $ command !! 6) pass
    | (command !! 0) == "reverse" = rev (read $ command !! 2) (read $ command !! 4) pass
    | (command !! 0) == "move" = move (read $ command !! 2) (read $ command !! 5) pass

main = do
    input <- readFile "input.txt"
    putStrLn $ foldl decode "fbgdceah" $ map words $ reverse $ lines input
